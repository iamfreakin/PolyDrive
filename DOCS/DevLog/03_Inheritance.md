...
## 1. 상속 (Inheritance)
기존의 클래스를 물려받아 새로운 기능을 추가하거나 수정하는 것입니다.
```cpp
// Bus.h
...
```
- `override` 키워드: "이 함수는 부모의 함수를 재정의한 거야!"라고 명시하며, 오타 등을 컴파일러가 잡아줍니다.
- 부모의 기능을 호출(`Car::ShowSpec()`)하면서 자식만의 고유한 내용(`[SportsCar]`)을 덧붙여 중복을 제거합니다.
...
컴퓨터는 `myVehicle`이 그냥 `Car*` 타입인 것만 알지만, 실행 시점에 실제 객체가 무엇인지 찾아가서 그에 맞는 함수를 호출합니다. 이것이 OOP의 꽃인 **동적 바인딩(Dynamic Binding)**입니다.
